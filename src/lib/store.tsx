...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  // A blocked storage API must not leave the application stuck on its splash.
  useEffect(() => {
    let savedTheme: Theme = "light";
    try {
      if (window.localStorage.getItem("playpoints.theme") === "dark") savedTheme = "dark";
    } catch {
      // Private/embedded browsers can deny storage; the app still opens in memory.
    }
    setTheme(savedTheme);
  ...
  // Do not overwrite the saved preference before hydration has read it.
  useEffect(() => {
    if (!ready) return;
    ...
    try {
      ...
      /* Storage may be unavailable; keep the current session usable. */
    }
  }, [theme, ready]);
