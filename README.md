...
## Deploy in 2 minutes (Vercel — recommended, free)

No config files needed — Vercel detects Next.js automatically.

1. **Push this project to GitHub** (create a repo and upload the files — `node_modules` and `.next` don't need to be uploaded, they're in `.gitignore`).
2. Go to **[vercel.com/new](https://vercel.com/new)** and sign in with your GitHub account.
3. Click **Import** on your repository → leave all default settings → click **Deploy**.

Done — your app is live at `https://<your-project>.vercel.app`, and **every future `git push` to `main` auto-deploys**. The URL is HTTPS, so Chrome's **Install app / Add to Home screen** works there too.

> No database or environment variables are required — all user data lives in the browser's localStorage.
>
> Optional: once your repo exists on GitHub, you can add a one-click deploy button to this README using
